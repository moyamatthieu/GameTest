...
export class Puppet extends THREE.Group {
    ...
    private slots: Map<PuppetSlot, THREE.Group[]> = new Map();
    
    // Sockets d'attachement
    private sockets: Map<PuppetSocket, THREE.Group> = new Map();
    ...
    // État de l'animation
    public state: PuppetState = 'IDLE';
    public isActionPlaying: boolean = false;
    private animationTime: number = 0;
    private currentSpeed: number = 0;
    
    // Valeurs cibles pour le lerping des rotations (pour des transitions fluides)
    private targetRotations: Map<THREE.Group, THREE.Euler> = new Map();
    ...
    private createBox(w: number, h: number, d: number, color: number, opacity: number = 1): THREE.Mesh {
    ...
    private registerSlot(slot: PuppetSlot, group: THREE.Group) {
    ...
    private initStructure() {
        ...
        lUArmMesh.position.y = -0.12; // Léger chevauchement pour éviter les trous
        lUArmSlot.add(lUArmMesh);

        this.leftLowerArm = new THREE.Group();
        this.leftLowerArm.name = "LeftLowerArm";
        this.leftLowerArm.position.y = -0.26; // Ajusté pour le chevauchement
        ...
        lLArmMesh.position.y = -0.11; // Léger chevauchement
        lLArmSlot.add(lLArmMesh);

        this.leftHand = new THREE.Group();
        this.leftHand.name = "LeftHand";
        this.leftHand.position.y = -0.24; // Ajusté
        ...
        rLArmMesh.position.y = -0.11; // Léger chevauchement
        rLArmSlot.add(rLArmMesh);

        this.rightHand = new THREE.Group();
        this.rightHand.name = "RightHand";
        this.rightHand.position.y = -0.24; // Ajusté
        ...
        this.leftUpperLeg.position.set(-0.12, -0.06, 0); // Ajusté
        ...
        lLLegMesh.position.y = -0.16; // Léger chevauchement
        lLLegSlot.add(lLLegMesh);

        this.leftFoot = new THREE.Group();
        this.leftFoot.name = "LeftFoot";
        this.leftFoot.position.y = -0.34; // Ajusté
        ...
        this.rightUpperLeg.position.set(0.12, -0.06, 0); // Ajusté
        ...
        rLLegMesh.position.y = -0.16; // Léger chevauchement
        rLLegSlot.add(rLLegMesh);

        this.rightFoot = new THREE.Group();
        this.rightFoot.name = "RightFoot";
        this.rightFoot.position.y = -0.34; // Ajusté
    ...
    private initAnimationSystem() {
        ...
        // Listener pour gérer la fin des actions prioritaires
        this.mixer.addEventListener('finished', (e) => {
            if (e.action.getClip().name === 'ATTACK' || e.action.getClip().name === 'HIT') {
     ...
     */
    public setEquipment(slot: PuppetSlot, mesh: THREE.Mesh | THREE.Group | null) {
     ...
     */
    public attachToSocket(socketName: PuppetSocket, object: THREE.Object3D) {
     ...
     * Vide un socket
     */
    public clearSocket(socketName: PuppetSocket) {
        const socket = this.sockets.get(socketName);
        if (socket) {
    ...
    private createAttackClip(): THREE.AnimationClip {
    ...
    private updateAnimationWeights(deltaTime: number) {
        // Logique de transition entre IDLE, WALK, RUN sur la Base Layer
        const walkAction = this.actions.get('WALK');
        const idleAction = this.actions.get('IDLE');

        const lerpFactor = Math.min(deltaTime * 10, 1.0); // Transition robuste basée sur le temps

        if (this.state === 'WALK' || this.state === 'RUN') {
            if (walkAction) {
                walkAction.enabled = true;
                walkAction.setEffectiveWeight(THREE.MathUtils.lerp(walkAction.weight, 1.0, lerpFactor));
            }
            if (idleAction) {
                idleAction.setEffectiveWeight(THREE.MathUtils.lerp(idleAction.weight, 0.0, lerpFactor));
            }
        } else {
            if (walkAction) {
                walkAction.setEffectiveWeight(THREE.MathUtils.lerp(walkAction.weight, 0.0, lerpFactor));
            }
            if (idleAction) {
                idleAction.enabled = true;
                idleAction.setEffectiveWeight(THREE.MathUtils.lerp(idleAction.weight, 1.0, lerpFactor));
            }
        }
    }

    public playAction(actionName: PuppetState) {
        const action = this.actions.get(actionName);
        if (action) {
            // Marquer comme action en cours pour empêcher l'écrasement par la locomotion
            if (actionName === 'ATTACK' || actionName === 'HIT' || actionName === 'CAST') {
    ...
    public setState(newState: PuppetState) {
