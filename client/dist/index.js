...
import { World, Position, Velocity, Input, MovementSystem, Renderable, CombatState, CombatSystem, Stats, Inventory, NPC, QuestState, CHAT_MESSAGE, Identity, Equipment } from 'shared';
...
socket.on('COMBAT_LOG', (event) => {
    const targetPos = world.getComponent(event.targetId, Position);
    if (targetPos) {
        let color = '#ff0000';
        let text = '';
...
socket.on('SERVER_STATE', (data) => {
    ...
    for (const entityData of data.entities) {
        let pos = world.getComponent(entityData.id, Position);
        let renderable = world.getComponent(entityData.id, Renderable);
        let stats = world.getComponent(entityData.id, Stats);
        let npc = world.getComponent(entityData.id, NPC);
        let identity = world.getComponent(entityData.id, Identity);
        if (!pos) {
            pos = new Position();
...
socket.on('INVENTORY_UPDATE', (data) => {
    if (playerEntity) {
        let inventory = world.getComponent(playerEntity, Inventory);
        if (!inventory) {
...
socket.on('EQUIPMENT_UPDATE', (data) => {
    if (playerEntity) {
        let equipment = world.getComponent(playerEntity, Equipment);
        if (!equipment) {
...
socket.on('STATS_UPDATE', (data) => {
    if (playerEntity) {
        let stats = world.getComponent(playerEntity, Stats);
        if (stats) {
...
socket.on('NPC_DIALOGUE', (data) => {
    console.log(`[${data.name}] ${data.text}`);
    uiManager.addChatMessage(data.name, data.text);
});
socket.on('QUEST_OFFERED', (quest) => {
...
socket.on('QUEST_UPDATE', (data) => {
    if (playerEntity) {
        let qs = world.getComponent(playerEntity, QuestState);
        if (!qs) {
            qs = new QuestState();
...
window.addEventListener('mousedown', (event) => {
    ...
    if (event.target.tagName !== 'CANVAS')
        return;
    if (!playerEntity || !playerCombatState)
        return;
